...
↓以下は変更されていると言える（text変数にChangeメソッドは存在するはずがないが疑似的に値の変更を再現した処理）
```go
// リスト1-2
text := "こんにちは"
...
リスト1-3のように値を直接変更するのではなく代入によって値の変更を表現する   
...
```
リスト3-1で運用後、ユーザー名にミドルネームを新たに追加するという仕様が入った場合はリスト3-1を以下のように修正する
```go
// リスト3-2
if fullNameA.FirstName == fullNameB.FirstName &&
...
値オブジェクトの場合、オブジェクトゆえにこの不自然さに気づきづらいが一度値に置き換えるとその不自然さに気づける
