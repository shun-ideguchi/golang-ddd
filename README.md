...
```
単に処理が内包されたセッターとは違い、メソッド名を見ればその振る舞いが分かるため、可読性が高い点もポイントである   
値オブジェクトとの違いは、交換（代入）によって変更を表現するのではなく、振る舞いが定義されたメソッドを通じて変化させる点である   
値オブジェクトと同様に異常値のチェックを行うため、不完全なオブジェクトが生成されることはない   
...
↓以下は同一性を判断するために識別子（identifier）を追加した例
```go
// リスト1-2 
package user
...
値オブジェクトの場合は全てのプロパティを比較し等価性を確認していたが、エンティティの場合、等価を保証するのは識別子である
```go
...
# サンプルコード
エンティティ・値オブジェクト → /app/internal/domain/user/*   
